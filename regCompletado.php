<?php
include './Classes/registraHG.php';
//Objeto
$reg = new registraHG();

//se asignan las variables del formulario anterior a los seters
$reg->setNombre($_POST['nombre']);
$reg->setApellido($_POST['apellido']);
$reg->setEmail($_POST['email']);
$reg->setEstado($_POST['estado']);
//$reg->setUsuario($_POST['usuario']);
$reg->setContra($_POST['contra']);
$reg->setDeacuerdo($_POST['deacuerdo']);
$reg->setFecha($_POST['fecha']);

//Se ejecuta la funcion para registrar los datos en la base de datos
$reg->registrar();
?>
...
<head>
  ...
  <meta charset="utf-8">
  <title>Registro - Hispano Green</title>
...
<div class="content">
  ...
  <p class="big grey">Ya puedes acceder a HispanoGreen.com con tu usuario y contraseña que ingresaste, Gracias!!. </p>

  <hr />
         <div class="form">
             <a href=""></a>
          <form method="get" id="searchform" action="Login.php" class="form-search">
            <!--<input type="text" value="" name="s" id="s" class="input-medium"/>-->
            <button type="submit" class="btn">Acceder a HispanoGreen.com</button>
          </form>
        </div>
